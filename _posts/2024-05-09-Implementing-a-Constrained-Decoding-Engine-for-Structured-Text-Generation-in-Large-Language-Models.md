...
- **Cache Possible Tokens**
    ...
    **Theoretical Discussion**
    ...
    This section analyzes the Theoretical of caching possible tokens and indicate why it is difficult. Feel free to skip this if you are less interested in theoretical aspects.

    Unlike regular expressions, the computational model equivalent to context-free grammar is [pushdown automata](https://en.wikipedia.org/wiki/Pushdown_automaton), which is not a finite state automaton. This implies that it is theoretically impossible to fully cache all states of a given context-free grammars. To make matter worse, even LL(k) or LR(k) grammars, subsets of context-free grammar that are equivalent to [deterministic pushdown automata](https://en.wikipedia.org/wiki/Deterministic_pushdown_automaton), may still not be simulated by a finite state automata since you can always add more symbols to the stack.

    So why bother caching possible tokens? There are mainly two reasons:

    1. Many grammars are not *that* complex.(There is a reason for Outlines and Guidance to only support regular expression initially.) Regular grammars, left-recursive grammars and right recursive grammars, empirically[^12], can be fully cached.

    2. The same grammar is likely used repeatedly to generate output and our Earley recognizer is likely to hit the same state again in the future.

    Indeed, there exist scenarios that does not satisfy any of the two conditions. That's why enabling cache support will be optional.

    **The Algorithm**

    The caching algorithm for Earley parsing is complex. Merely caching all Earley sets for a given input sequence is very inefficient. We need a method to compact Earley sets. For Earley sets `S(0), S(1), ..., S(m)`, we have the following observations:
...
#### Compute possible tokens, the core loop

```python
def compute_possible_tokens():
    if earley_sets in cache:
        return cache[earley_sets]
    current_index = len(earley_sets)-1
    new_set = earley_sets.add_new_set()
    possible_tokens = set()
    for token in all_tokens:
        while len(token) != 0:
            if token[0] not in possible_bytes[current_index]:
                break
            for item in earley_sets[current_index]:
                try_scan(item[0],new_set) # will skip dotted nonterminal
            current_index += 1
